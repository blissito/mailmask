...
- **Tema del sitio público** (sep-2026, dirección "lucha libre": crema + rojo de máscara + dorado sólo en la máscara, titulares en Bricolage Grotesque 800; el verde `mask-*` queda para la app y para estados OK): landing, pricing, login y registro usan tokens semánticos (`bg-bg`, `bg-bg-elev`, `text-fg`, `text-fg-muted`, `border-line`, `text-accent-text`, `bg-accent`) definidos en `public/css/input.css` y expuestos en `tailwind.config.ts`. Claro por default; `public/js/theme.js` (en `<head>`, sin `defer`) aplica `data-theme` guardado en `localStorage` y sin elección manda `prefers-color-scheme`. Piezas repetidas: `.btn-primary`, `.btn-secondary`, `.card`, `.mock`, `.chip`, `.eyebrow`, `.faq`. **No uses `zinc-*` en esas cuatro páginas**; la app (`/app`, `/bandeja`) sigue oscura con zinc y no depende de los tokens. Fuentes Inter + JetBrains Mono desde Google Fonts (la CSP ya lo permite). El video de Brenda en la home es lite-embed: thumbnail local y `frame-src` con `youtube-nocookie.com`.
